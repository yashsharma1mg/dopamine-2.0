...
export const componentManifests: ComponentManifest[] = [
  ...
  },
  {
    ...
    api: [
      ...
      { name: "height", type: "number", defaultValue: "—", description: "Fixed sheet height in px (clamped 128–600; min 128, max 600). Omit for content-driven sizing." },
  ...
  },
  {
    slug: "product",
    name: "Product",
    summary: "PDP product hero: an image carousel, then title / brand / composition, and an optional green alternative pill.",
    category: "Display",
    status: "ready",
    storyId: "components-product--playground",
    packageName: "@dopamine2.0/ui",
    importExample: 'import { Product } from "@dopamine2.0/ui";',
    anatomy: ["Image carousel", "Title (title-22)", "Brand", "Composition + underlined value", "Optional alternative pill"],
    variants: ["with alternative pill", "without"],
    sizes: ["360px width"],
    states: ["alternative on/off"],
    usage: { do: ["Lead with the product image carousel and title.", "Use the alternative pill to surface a cheaper generic."], dont: ["Do not omit the composition on a medicine PDP."] },
    accessibility: ["Carousel scrolls horizontally.", "The alternative pill is a labelled button."],
    contentGuidance: ["Title is the product name; brand is the manufacturer."],
    api: [
      { name: "title", type: "string", defaultValue: "Telma 40 Tablet", description: "Product name." },
      { name: "brand", type: "string", defaultValue: "—", description: "Manufacturer." },
      { name: "compositionValue", type: "string", defaultValue: "Telmisartan (40mg)", description: "Underlined composition." },
      { name: "alternative", type: "boolean", defaultValue: "true", description: "Show the green alternative pill." },
      { name: "images", type: "ReactNode[]", defaultValue: "2 placeholders", description: "Carousel image slots." }
    ],
    links: { figma: "https://www.figma.com/design/BsQQUym4xOYfOs419MpBBX/Components-%7C-Dopamine-2.0?node-id=6825-4912", specification: "/specs/Product.md", audit: "", source: "/storybook/?path=/docs/components-product--docs" }
  },
  {
    slug: "product-label",
    name: "ProductLabel",
    summary: "PDP feature strip: a row of trust/feature badges (icon + bold title / secondary description), separated by dividers.",
    category: "Display",
    status: "ready",
    storyId: "components-productlabel--playground",
    packageName: "@dopamine2.0/ui",
    importExample: 'import { ProductLabel } from "@dopamine2.0/ui";',
    anatomy: ["Feature item (icon + title + description)", "Vertical dividers"],
    variants: ["configurable items"],
    sizes: ["360px width"],
    states: ["optional dotted underline per item"],
    usage: { do: ["Use for trust/feature signals (Genuine, Prescription, popularity)."], dont: ["Do not overload with more than 3–4 items."] },
    accessibility: ["Icons are decorative; titles carry meaning."],
    contentGuidance: ["Keep titles to one or two words."],
    api: [{ name: "items", type: "ProductLabelItem[]", defaultValue: "Genuine / Prescription / popularity", description: "Feature badges (icon, title, description, underline?)." }],
    links: { figma: "https://www.figma.com/design/BsQQUym4xOYfOs419MpBBX/Components-%7C-Dopamine-2.0?node-id=6825-4914", specification: "/specs/ProductLabel.md", audit: "", source: "/storybook/?path=/docs/components-productlabel--docs" }
  },
  {
    slug: "add-to-cart-pill",
    name: "AddToCartPill",
    summary: "PDP add-to-cart bar: price (effective / struck MRP / discount) and an ADD button; default or compact sticky.",
    category: "Cart",
    status: "ready",
    storyId: "components-addtocartpill--playground",
    packageName: "@dopamine2.0/ui",
    importExample: 'import { AddToCartPill } from "@dopamine2.0/ui";',
    anatomy: ["Price (effective / struck MRP / discount)", "Pack + tax note (default)", "ADD button"],
    variants: ["default", "sticky"],
    sizes: ["360px width"],
    states: ["default (with pack/tax note) vs sticky (compact)"],
    usage: { do: ["Use `sticky` for the pinned bottom bar.", "Keep the struck MRP next to the price."], dont: ["Do not hide the effective price."] },
    accessibility: ["ADD is the DS Button."],
    contentGuidance: ["Discount reads like '7% off'."],
    api: [
      { name: "state", type: "default | sticky", defaultValue: "default", description: "Layout variant." },
      { name: "price", type: "string", defaultValue: "₹371", description: "Effective price." },
      { name: "mrp", type: "string", defaultValue: "₹100", description: "Struck MRP." },
      { name: "discount", type: "string", defaultValue: "7% off", description: "Discount label." },
      { name: "onAdd", type: "() => void", defaultValue: "—", description: "ADD callback." }
    ],
    links: { figma: "https://www.figma.com/design/BsQQUym4xOYfOs419MpBBX/Components-%7C-Dopamine-2.0?node-id=6825-6198", specification: "/specs/AddToCartPill.md", audit: "", source: "/storybook/?path=/docs/components-addtocartpill--docs" }
  },
  {
    slug: "offer",
    name: "Offer",
    summary: "PDP offers section: an optional promo banner and a 'Save more with additional offers' list, closed by a See-all action.",
    category: "Display",
    status: "ready",
    storyId: "components-offer--playground",
    packageName: "@dopamine2.0/ui",
    importExample: 'import { Offer } from "@dopamine2.0/ui";',
    anatomy: ["Optional banner", "Offers list (icon + title + subtitle + chevron)", "See all offers action"],
    variants: ["with banner", "list only"],
    sizes: ["360px width"],
    states: ["banner optional"],
    usage: { do: ["Use to surface payment/cashback offers on the PDP."], dont: ["Do not bury the primary price below the offers."] },
    accessibility: ["Offer rows and See-all are buttons."],
    contentGuidance: ["Row title is the offer; subtitle repeats the detail."],
    api: [
      { name: "banner", type: "ReactNode", defaultValue: "—", description: "Optional promo banner slot." },
      { name: "offers", type: "OfferRow[]", defaultValue: "3 sample offers", description: "Offer rows (icon, title, subtitle)." },
      { name: "onSeeAll", type: "() => void", defaultValue: "—", description: "See-all callback." }
    ],
    links: { figma: "https://www.figma.com/design/BsQQUym4xOYfOs419MpBBX/Components-%7C-Dopamine-2.0?node-id=6825-5309", specification: "/specs/Offer.md", audit: "", source: "/storybook/?path=/docs/components-offer--docs" }
  },
  {
    slug: "product-information",
    name: "ProductInformation",
    summary: "PDP information accordion: collapsible sections (title + rotating chevron) with an optional See more / See less read-more.",
    category: "Display",
    status: "ready",
    storyId: "components-productinformation--playground",
    packageName: "@dopamine2.0/ui",
    importExample: 'import { ProductInformation } from "@dopamine2.0/ui";',
    anatomy: ["Section header (title + chevron)", "Collapsible body", "Optional See more / See less", "8px divider bands"],
    variants: ["configurable sections"],
    sizes: ["360px width"],
    states: ["expanded / collapsed per section; read-more clamped / expanded"],
    usage: { do: ["Use for long PDP content split into named sections.", "Mark long sections `readMore`."], dont: ["Do not open every section by default."] },
    accessibility: ["Headers are buttons with aria-expanded; the chevron rotates on expand."],
